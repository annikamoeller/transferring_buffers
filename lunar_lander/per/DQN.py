...
class DQN():
  ...
  def train(self, inputs, targets, importances, indices, errors):
    self.buffer.set_priorities(indices, errors)
    batch_size = len(targets)
    inputs = np.array(inputs).reshape(batch_size, self.state_space)
    targets = np.array(targets)
    self.model.fit(inputs, targets, sample_weight=importances**(0.95), epochs=1, batch_size=batch_size, verbose=0)#was 1-self.current_epsilon
  ...
  def calculate_inputs_and_targets_ddqn(self, experiences):
    ...
    for index, experience in enumerate(experiences):
      ...
      errors.append(abs(target_val - best_action_q_value))
