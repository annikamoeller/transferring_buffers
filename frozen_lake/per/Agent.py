...
class DQN():
    ...
    def train(self, experience, batch_size):
        batch, importances, indices = self.buffer.sample(batch_size) # get importances
        ...
        for index, experience in enumerate(batch):
            ...
            errors.append(abs(target - best_action))

        self.buffer.set_priorities(indices, errors) # set the sampling priorities based on error
        targets = np.array(targets)
        inputs = np.array(states).reshape(batch_size, self.state_space)
        # fit model with importances as weight for each sample 
        self.model.fit(inputs, targets, sample_weight=importances**(1-self.current_epsilon),
